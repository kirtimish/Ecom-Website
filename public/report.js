...
window.addEventListener('DOMContentLoaded',async (event) => {
    ...
    try {
        const res = await axios.get('http://52.202.41.22:3000/getAllUrl',{headers: {'Authorization' : token}})
...
download.addEventListener('click',async(event) => {
    ...
    try {
        let response = await axios.get('http://52.202.41.22:3000/download', {headers:{'Authorization' : token}})
